...
@section('title', 'Tambah Aturan Partuturan - Tarombo')
@section('styles')
        ...
        <div class="col-md-12">
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">Tambah Aturan Partuturan</h5>
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.partuturan-rules.store') }}" method="POST">
                        @csrf
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <select class="select2 form-select @error('partuturan_term_id') is-invalid @enderror" id="partuturan_term_id" name="partuturan_term_id">
                                    ...
                                    @foreach($terms as $term)
                                        <option value="{{ $term->id }}" data-category="{{ $term->category->name }}" {{ old('partuturan_term_id', $preSelectedTerm ? $preSelectedTerm->id : '') == $term->id ? 'selected' : '' }}>
                            ...
                            <div class="col-sm-10">
                                <select class="select2 form-select @error('relationship_pattern_id') is-invalid @enderror" id="relationship_pattern_id" name="relationship_pattern_id">
                                    ...
                                    @foreach($patterns as $pattern)
                                        <option value="{{ $pattern->id }}" data-pattern="{{ $pattern->pattern }}" {{ old('relationship_pattern_id', $preSelectedPattern ? $preSelectedPattern->id : '') == $pattern->id ? 'selected' : '' }}>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <div class="form-check form-check-inline mt-2">
                                    <input class="form-check-input @error('ego_gender') is-invalid @enderror" type="radio" name="ego_gender" id="ego_male" value="male" {{ old('ego_gender', 'male') == 'male' ? 'checked' : '' }}>
                                    <label class="form-check-label" for="ego_male">Laki-laki</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input @error('ego_gender') is-invalid @enderror" type="radio" name="ego_gender" id="ego_female" value="female" {{ old('ego_gender') == 'female' ? 'checked' : '' }}>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <div class="form-check form-check-inline mt-2">
                                    <input class="form-check-input @error('relative_gender') is-invalid @enderror" type="radio" name="relative_gender" id="relative_male" value="male" {{ old('relative_gender', 'male') == 'male' ? 'checked' : '' }}>
                                    <label class="form-check-label" for="relative_male">Laki-laki</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input @error('relative_gender') is-invalid @enderror" type="radio" name="relative_gender" id="relative_female" value="female" {{ old('relative_gender') == 'female' ? 'checked' : '' }}>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initialize displays if values exist (e.g., on validation error)
        if (termSelect.value) {
