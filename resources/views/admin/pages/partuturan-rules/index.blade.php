...
@section('title', 'Aturan Partuturan - Tarombo')
@section('styles')
        ...
        <div class="card-datatable table-responsive pt-0">
            <div class="card-header d-flex justify-content-between align-items-center">
                <div class="head-label">
                    <h5 class="card-title mb-0">Aturan Partuturan</h5>
                </div>
                <div class="dt-action-buttons text-end pt-3 pt-md-0">
                    <div class="dt-buttons btn-group flex-wrap">
                        <div class="btn-group">
                            <a href="{{ route('admin.partuturan-rules.create') }}" class="btn btn-secondary create-new btn-primary waves-effect waves-light">
                                <span>
...
<script>
    $(document).ready(function () {
        $('.datatables').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('admin.partuturan-rules.index') }}",
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false},
                {data: 'term', name: 'term'},
                {data: 'pattern', name: 'pattern'},
                {data: 'ego_gender_label', name: 'ego_gender_label'},
                {data: 'relative_gender_label', name: 'relative_gender_label'},
        ...
        $('.datatables').on('click', '.delete-record', function() {
            ...
            Swal.fire({
                title: "Apakah Anda yakin menghapus aturan ini?",
            ...
            .then(res => {
                if(res.isConfirmed) {
                    axios({
                        method: 'delete',
                        url: '{{ route('admin.partuturan-rules.destroy', '') }}/' + id,
                        responseType: 'json'
                    })
